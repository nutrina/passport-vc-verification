<script lang="ts">
  ...
  import { ethers, type Signature, type Signer } from "ethers";
  ...
  export const getSerializedSignedVC = async ({
    ...
    const fullDocument = generateFullDocument(await signer.getAddress(), document);
  ...
  export type FullDocument = ExampleDocument & {
    issuer: string;
    issuanceDate: string;
